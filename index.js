...
let mexEnemyHealth = 75; // Maximum health an enemy can have
let enemyAttackDamagToHero = 25; // Maximum damage an enemy can inflict
// player variable
let playerHealth = 100;
let playerAttackDamagToEnemy = 50; // Maximum damage the player can inflict
let numHealthPotion = 3; // Initial number of health potions
let healthPotionHealAmount = 30; // Amount of health restored by a health potion
let healthPotionDropChance = 50; // Percentage chance of a health potion dropping after defeating an enemy
// while loop condition
let gameRunning = true;
console.log(chalk.bold.bgBlue("\n\t\tWelcome to DeadZone!"));
// Main game loop
Game: while (gameRunning) {
    let enemyHealth = Math.floor(Math.random() * mexEnemyHealth + 1); // randomly genrat enemyHealth
    let enemyIndex = Math.floor(Math.random() * enemies.length); // Select a random enemy
    let enemy = enemies[enemyIndex]; // Get enemy name
    ...
    while (enemyHealth > 0) {
        ...
        if (options.ans === "1. Attack") {
            ...
            console.log(chalk.magenta(`you strike the ${enemy} for ${damagToEnemy} Damage.`));
            console.log(chalk.blue(`${enemy}  strike you for ${damagPlayer} Damage.`));
            // Check if player is defeated
            if (playerHealth < 1) {
                console.log(chalk.green("you have too much damage. you are to weak to continue."));
                break;
            }
        }
            ...
            if (numHealthPotion > 0) {
                ...
                console.log(chalk.magenta(`you have ${numHealthPotion} health potion left.`));
            }
            else {
